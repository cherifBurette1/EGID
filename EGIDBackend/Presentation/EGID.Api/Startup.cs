...
using EGID.Api.Background;
using EGID.Api.Notifiers;
...
namespace EGID.Api
{
    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            //.UseRedisStorage(_redisHangfire, new Hangfire.Redis.RedisStorageOptions()
            //{
            ...
            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new OpenApiInfo
                {
                    Version = "v1",
                    Title = "EGID.Api",
                    Description = "Swagger document for EGID Apis",
                });
                var xmlFile = $"{Assembly.GetExecutingAssembly().GetName().Name}.xml";
                var xmlPath = Path.Combine(AppContext.BaseDirectory, xmlFile);
                c.IncludeXmlComments(xmlPath);
                //c.IncludeXmlComments(string.Format(@"{0}\bin\EGID.Api.xml", Directory.GetCurrentDirectory()));
                //c.DocumentFilter<AuthenticationTokenOperation>(););
            });

            services.AddSignalR()
                .AddStackExchangeRedis(Configurations.APISignalRBackPlaneConnectionString, options =>
                {
                    options.Configuration.ChannelPrefix = $"EGIDBackPlane-{_env.EnvironmentName}";
                });
        ...
        public void ConfigureIoC(IServiceCollection services)
        {
            ...
            services.AddDbContext<IEGIDAuditDbContext, EGIDAuditDbContext>(o => o.UseSqlServer(Configurations.EGIDAuditConnectionString));


            services.AddCoreService();
            services.AddInfrastructure();

            services.AddTransient<IStockNotifier, StockConcreteNotifier>();
            services.AddTransient(provider => new Lazy<IStockNotifier>(provider.GetService<IStockNotifier>));


            services.AddTransient<IEGIDBackgroundConfiguration, EGIDBackgroundConfiguration>();
            services.AddTransient(provider => new Lazy<IEGIDBackgroundConfiguration>(provider.GetService<IEGIDBackgroundConfiguration>));
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IEGIDBackgroundConfiguration EGIDBackgroundConfiguration,
            IEGIDEntities context, IBackgroundSchedular schedular)
        {
            ...
            if (env.IsDevelopment() || env.IsEnvironment("DockerQA"))
            {
                ...
                app.UseSwaggerUI(c =>
                {
                    c.SwaggerEndpoint("/swagger/v1/swagger.json", "EGID Documentation");
                    ...
                    c.IndexStream = () => GetType().GetTypeInfo().Assembly.GetManifestResourceStream("EGID.Api.SwaggerIndex.html");
                });
            ...
            EGIDBackgroundConfiguration.StartJobs(context, schedular);
        }
    }
}
